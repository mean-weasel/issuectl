...
terminal:
  app: ghostty           # terminal emulator to use
  mode: window           # "window" (new window) or "tab" (new tab in existing)

worktrees:
  dir: ~/.issuectl/worktrees/  # where git worktrees are created
...
- **Branch name** — pre-filled with `issue-{number}-{slug}`, editable
- **Context toggles** — checkboxes to include/exclude individual comments and referenced files
- **Custom preamble** — optional text field for additional instructions to prepend to the prompt
...
1. **Assemble context:** Issue title, body, selected comments, and selected referenced files
2. **Check history:** If `issuectl:deployed` label exists, show deployment history (branch, PR, review comments) and confirm re-launch
3. **Prepare workspace:** Depending on workspace mode:
   - *Existing repo:* checkout default branch, pull latest, create issue branch
   - *Worktree:* create worktree at temp path, checkout issue branch
   - *Fresh clone:* clone repo to temp path, checkout issue branch
4. **Apply label:** Add `issuectl:deployed` to the issue on GitHub
5. **Open terminal:** Launch Claude Code in the workspace directory (see Terminal configuration below)

### Terminal configuration

How the tool opens Ghostty is **configurable in settings** (default: new window):
...
```bash
cd {workspace_path} && git checkout {branch_name} && claude "{assembled_issue_context}"
```

Implementation details of Ghostty process spawning (CLI flags, IPC) are deferred to the implementation plan.
...
- Content of selected referenced files from the issue body (detected by regex matching file path patterns and GitHub blob URLs, individually toggleable)
...
- **Refresh button** in the toolbar to manually re-fetch issues and PRs for this repo from GitHub (bypasses cache TTL)
