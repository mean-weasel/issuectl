...
describe("seedDefaults", () => {
  ...
  it("inserts all default settings", () => {
    seedDefaults(db);
    const settings = getSettings(db);
    expect(settings).toHaveLength(20);
    ...
    expect(keys).toContain("webhook_raw_payload_retention_days");
    expect(keys).toContain("webhook_event_retention_days");
  ...
  it("seeds webhook defaults", () => {
    ...
    expect(getSetting(db, "webhook_raw_payload_retention_days")).toBe("7");
    expect(getSetting(db, "webhook_event_retention_days")).toBe("30");
